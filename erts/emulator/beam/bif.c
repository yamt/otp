...
{
    int decimals = 6;   /* Same default as printf(3) */
    int compact = 0;
    Eterm list = BIF_ARG_2;
    Eterm arg;
    int i;
    ...
    while (is_list(list)) {
        ...
        if (arg == am_compact)
            ...
            if (*tp == make_arityval(2) && tp[1] == am_decimals && is_small(tp[2]))
    ...
    GET_DOUBLE(BIF_ARG_1, f);
    if ((i = sys_double_to_chars_fast(f.fd, fbuf, sizeof(fbuf), decimals, compact)) <= 0)
        BIF_ERROR(BIF_P, EXC_INTERNAL_ERROR);
    need = i*2;
    hp = HAlloc(BIF_P, need);
    BIF_RET(buf_to_intlist(&hp, fbuf, i, NIL));
